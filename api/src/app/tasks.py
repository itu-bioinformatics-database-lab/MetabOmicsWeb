...
def save_analysis(
    ...
    try:
        ...
        if X_train_transformed and len(X_train_transformed) > 0:
            ...
            logger.warning(
                '[DEBUG] X_train_transformed[0] first 5 entries: %s',
                dict(list(first_sample.items())[:5])
            )
        ...
        logger.warning('[DEBUG] raw y parameter value: %s', y)
        logger.warning('[DEBUG] raw y type: %s', type(y))

        # Normalize y into a single-sample label list like ['healthy']
        ...
        logger.warning(
            ...
            len(results_diff_score) if results_diff_score else 'None'
        )
        ...
        if results_diff_score and len(results_diff_score) > 0:
            ...
            if len(nonzero_diffs) == 0:
                logger.warning('[DEBUG] WARNING: All reaction outputs are zero!')
            else:
                logger.warning(
                    '[DEBUG] First 5 non-zero reaction outputs: %s',
                    dict(list(nonzero_diffs.items())[:5])
                )

        results_diff_score_stripped = [dict((k.replace("_max", "").replace("_min", ""), v) for k, v in s.items()) for s in results_diff_score]
        results_pathway = pathway_scaler.transform(results_diff_score_stripped)
